...
avg_price_by_property_type <- cleaned_listings %>%
  filter(property_type != "Room in hotel") %>%
  group_by(property_type) %>%
  summarize(avg_price = mean(price)) 

# Sort the results in descending order of average price and select the top 10 property types
most_expensive_property_types <- avg_price_by_property_type %>%
  arrange(desc(avg_price)) %>%
  head(10)

# Create a bar chart of the average price for each property type
ggplot(most_expensive_property_types, aes(x = property_type, y = avg_price)) +
  geom_bar(stat = "identity", fill = "blue") +
  xlab("Property Type Code") +
  ylab("Average Price") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5, size = 10)) +
  ggtitle("Top 10 Most Expensive Property Types")
...
cleaned_listings <- cleaned_listings %>% 
  filter(!(neighbourhood_cleansed %in% c("Mc unclaimed #13", "Mc unclaimed #11")))
...
cleaned_listings %>%
  ...
  slice_head(n = 2) %>%
  ggplot() +
  geom_bar(mapping = aes(x = fct_rev(fct_infreq(neighbourhood_cleansed)), y = price), stat = "identity") +
  labs(x = "Neighborhood", y = "Price", title = "Top 2 High Price Listings Across All Neighborhoods") +
  coord_flip()
